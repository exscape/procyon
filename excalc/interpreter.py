...
def evaluate_expr(expr):
	...
	lex_lexer = lex.lex(module=lexer, debug=True, optimize=False)
	yacc_parser = yacc.yacc(module=parser, debug=True, start="toplevel", debugfile="yaccdebug")
	parse_tree = yacc_parser.parse(expr, lexer=lex_lexer, debug=1)#__debugparse__)
...
def __evaluate_tree(tree, scope):
	...
	if kind == "binop":
		...
		if not func_name in __functions and not _var_exists(scope, func_name):
		...
		if cmd_name == 'vars':
			...
			vars = [v for v in __global_scope[1] if (v != '_' and not (v in __initial_state and __initial_state[v] == __global_scope[1][v]))]
...
def __evaluate_function(func, args, scope):
	...
	if len(args) != len(params):
		# TODO: Should probably not be SyntaxError
		raise SyntaxError('Attempted to call {}() with {} arguments, exactly {} required'.format(name, len(args), len(params)))
